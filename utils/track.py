...
def memo_add(args, env_memo_rec, row, col):
    ...
    env_memo_rec = np.where(env_memo_rec > args.bound * lim["memo"], args.bound * lim["memo"], env_memo_rec)

    return env_memo_rec

def memo_minus(args, env_memo_rec):
    if args.neg_memo:
        env_memo_rec = np.where(env_memo_rec > -1 * lim["memo"], env_memo_rec - 1, env_memo_rec)
    else:
